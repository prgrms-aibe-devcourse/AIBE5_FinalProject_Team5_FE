...
// 일정 페이지 (캘린더·리마인더·Google Calendar 연동)
export default function SchedulePage() {
    ...
    calendarStatus?.googleUser && !isLoadingStatus ? (
      calendarStatus.connected ? (
        <DashboardActionButton
          label={isCalendarActionLoading ? '해제 중...' : 'Google Calendar 연동 해제'}
          variant="secondary"
          onClick={() => void handleDisconnectGoogleCalendar()}
          disabled={isCalendarActionLoading}
        />
      ) : (
        <DashboardActionButton
          label={isCalendarActionLoading ? '연동 중...' : 'Google Calendar 연동'}
  ...
  return (
    <DashboardShell title="일정" action={googleCalendarAction}>
      {statusError ? (
